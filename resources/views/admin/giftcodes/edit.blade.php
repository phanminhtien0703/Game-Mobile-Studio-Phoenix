...
@section('title', 'Sửa Giftcode | Game Mobile Studio')
                ...
                <div class="col-12">
                  <div class="card">
                    <div class="card-header">
                      <h5 class="card-title">Sửa Giftcode</h5>
                    </div>
                    <div class="card-body">
                      @if ($errors->any())
                      ...
                      <form action="{{ route('admin.giftcodes.update', $giftcode->giftcode_id) }}" method="POST">
                        @csrf
                        @method('PUT')
                          ...
                          <select class="form-select @error('game_id') is-invalid @enderror" id="game_id" name="game_id" required>
                            ...
                            @foreach($games as $game)
                              <option value="{{ $game->game_id }}" 
                                      ...
                                      {{ $giftcode->game_id == $game->game_id ? 'selected' : '' }}>
                                {{ $game->game_name }}
                              </option>
                            @endforeach
                          </select>
                        ...
                        <div class="mb-3">
                          ...
                          <textarea class="form-control @error('message') is-invalid @enderror" id="message" name="message" rows="3" placeholder="VD: Xin chào Admin, tôi muốn nhận giftcode cho game [Tên game]. Cảm ơn Admin!" required>{{ old('message', $giftcode->message) }}</textarea>
                        ...
                        <div class="mb-3">
                          ...
                          <input type="number" class="form-control @error('total_quantity') is-invalid @enderror" id="total_quantity" name="total_quantity" placeholder="VD: 500" value="{{ old('total_quantity', $giftcode->total_quantity) }}" min="1" required>
                        ...
                        <div class="mb-3">
                          ...
                          <input type="number" class="form-control @error('used_quantity') is-invalid @enderror" id="used_quantity" name="used_quantity" placeholder="VD: 0" value="{{ old('used_quantity', $giftcode->used_quantity) }}" min="0" required>
                          <small class="form-text text-muted">Số lượng giftcode đã được sử dụng</small>
                          @error('used_quantity')
                            <span class="invalid-feedback">{{ $message }}</span>
                          @enderror
                        </div>
                        ...
                        <input type="hidden" id="logo_game_url" name="logo_game_url" value="{{ old('logo_game_url', $giftcode->logo_game_url) }}">
